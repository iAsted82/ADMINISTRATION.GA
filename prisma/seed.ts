...
async function main() {
  ...
  const users = [
    ...
    {
      email: 'agent.etatcivil@admin.ga',
      password: await bcrypt.hash('AgentEC2024!', 12),
      firstName: 'Paul',
      lastName: 'OBIANG',
      role: UserRole.AGENT,
      organizationId: mairieLibreville?.id,
  ...
  const agentEtatCivil = await prisma.user.findUnique({ where: { email: 'agent.etatcivil@admin.ga' } });

  if (jeanUser && agentEtatCivil && mairieLibreville) {
    await prisma.serviceRequest.create({
      data: {
        type: 'ACTE_NAISSANCE',
        status: 'IN_PROGRESS',
        submittedById: jeanUser.id,
        assignedToId: agentEtatCivil.id,
        organizationId: mairieLibreville.id,
        formData: {
          firstName: 'Jean',
          lastName: 'DUPONT',
